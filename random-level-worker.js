  ...
  this.getValue = function (d0, d1) {
    ...
    return lerp(
      ...
      lerp(
        ...
        lerp(
          d5,
          grad(this.p[i1], d4, d3, d2),
          grad(this.p[j], d4 - 1.0, d3, d2)
        ),
        lerp(
          d5,
          grad(this.p[l], d4, d3 - 1.0, d2),
          grad(this.p[i], d4 - 1.0, d3 - 1.0, d2)
        )
      ),
      lerp(
        d6,
        lerp(
          d5,
          grad(this.p[i1 + 1], d4, d3, d2 - 1.0),
          grad(this.p[j + 1], d4 - 1.0, d3, d2 - 1.0)
        ),
        lerp(
          d5,
          grad(this.p[l + 1], d4, d3 - 1.0, d2 - 1.0),
          grad(this.p[i + 1], d4 - 1.0, d3 - 1.0, d2 - 1.0)
        )
      )
    );
  };
  ...
  this.createLevel = function (seed, xSize, zSize, ySize) {
    ...
    this.grow = function (aint) {
      ...
      for (var l = 0; l < i; ++l) {
        ...
        for (var i1 = 0; i1 < j; ++i1) {
          ...
          var j1;
          var k1 = parseInt(
            ((j1 = parseInt(aint[l + i1 * i], 10)) * this.zSize + i1) *
              this.xSize +
              l,
            10
          );
          ...
          if (
            (l1 =
              parseInt(
                this.tiles[((j1 + 1) * this.zSize + i1) * this.xSize + l],
                10
              ) & 255) == 7 &&
            j1 <= k / 2 - 1 &&
            flag1
          ...
          for (var i2 = 0; i2 < 20; ++i2) {
            ...
            if (k1 >= 0 && l1 >= 0 && k1 < this.xSize && l1 < this.zSize) {
                ...
                for (i3 = k1 - b0; i3 <= k1 + b0 && flag; ++i3) {
                  for (j3 = l1 - b0; j3 <= l1 + b0 && flag; ++j3) {
                    if (
                      ...
                      if (
                        (this.tiles[(l2 * this.zSize + j3) * this.xSize + i3] &
                          255) !=
                        0
                      ) {
                        flag = false;
              ...
              if (flag) {
                ...
                if (
                  (this.tiles[((j2 - 1) * this.zSize + l1) * this.xSize + k1] &
                    255) ==
                    1 &&
                  j2 < this.ySize - k2 - 1
                ) {
                    ...
                    for (var l3 = k1 - k3; l3 <= k1 + k3; ++l3) {
                      ...
                      for (var j4 = l1 - k3; j4 <= l1 + k3; ++j4) {
                        ...
                        if (
                          Math.abs(i4) != k3 ||
                          Math.abs(k4) != k3 ||
                          (random.nextInt(2) != 0 && j3 != 0)
                        ) {
                          this.tiles[
                            (i3 * this.zSize + j4) * this.xSize + l3
                          ] = 14; //(byte) Tile.leaves.id;
                        }
                      }
                    }
                  }
    ...
    this.placeOre = function (tile, j, k, l) {
      ...
      for (var l1 = 0; l1 < k1; ++l1) {
        ...
        var i2 = parseInt(
          ((random.nextFloat() + random.nextFloat()) * 75.0 * j) / 100.0,
          10
        );
        ...
        for (var j2 = 0; j2 < i2; ++j2) {
          ...
          for (var k2 = Math.round(f - f7); k2 <= Math.round(f + f7); ++k2) {
            for (
              var l2 = Math.round(f1 - f7);
              l2 <= Math.round(f1 + f7);
              ++l2
            ) {
              for (
                var i3 = Math.round(f2 - f7);
                i3 <= Math.round(f2 + f7);
                ++i3
              ) {
                var f8 = k2 - f;
                ...
                if (
                  ...
                  var j3 = parseInt(
                    (l2 * this.zSize + i3) * this.xSize + k2,
                    10
                  );
    ...
    for (i1 = 0; i1 < l; ++i1) {
      ...
      for (var l3 = 0; l3 < i3; ++l3) {
        ...
        if (random.nextFloat() >= 0.3) {
          ...
          for (
            var i4 = parseInt(f8 - f11, 10);
            i4 <= parseInt(f8 + f11, 10);
            ++i4
          ) {
            for (
              var j4 = parseInt(f9 - f11, 10);
              j4 <= parseInt(f9 + f11, 10);
              ++j4
            ) {
              for (var k4 = f10 - f11; k4 <= f10 + f11; ++k4) {
                ...
                var f14 = k4 - f10;
                if (
                  f12 * f12 + f13 * f13 * 2.0 + f14 * f14 < f11 * f11 &&
                  i4 >= 1 &&
                  j4 >= 1 &&
                  k4 >= 1 &&
                  i4 < xSize - 1 &&
    ...
    for (i1 = 0; i1 < xSize; ++i1) {
      j5 =
        j5 +
        this.floodFill(i1, ySize / 2 - 1 + extray, 0, 0, l) +
        this.floodFill(i1, ySize / 2 - 1, zSize - 1 + extray, 0, l);
    }
    for (i1 = 0; i1 < zSize; ++i1) {
      j5 =
        j5 +
        this.floodFill(0, ySize / 2 - 1 + extray, i1, 0, l) +
        this.floodFill(xSize - 1, ySize / 2 - 1 + extray, i1, 0, l);
    }
    i1 = (xSize * zSize) / 200;
    for (l1 = 0; l1 < i1; ++l1) {
      if (l1 % 100 == 0) {
